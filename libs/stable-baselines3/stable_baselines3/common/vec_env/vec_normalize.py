...
class VecNormalize(VecEnvWrapper):
    ...
    def __init__(
        ...
        if isinstance(self.observation_space, gym.spaces.Dict):
            ...
            self.obs_spaces = self.observation_space.spaces
            self.obs_rms = [RunningMeanStd(shape=obs_space.shape) for obs_space in self.obs_spaces]
        else:
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        for idx, done in enumerate(dones):
            if isinstance(done, tuple) or isinstance(done, np.ndarray):
                if not any(done):
                    continue
            else:
                if not done:
    ...
    def normalize_obs(self, obs: Union[np.ndarray, Dict[str, np.ndarray]]) -> Union[np.ndarray, Dict[str, np.ndarray]]:
        ...
        if self.norm_obs:
            if isinstance(obs, dict) and isinstance(self.obs_rms, dict):
                ...
                obs_ = []
                for i, ob in enumerate(obs):
                    obs_.append(self._normalize_obs(ob, self.obs_rms[i]).astype(np.float32))
                obs_ = tuple(obs_)
            else:
    ...
    def unnormalize_obs(self, obs: Union[np.ndarray, Dict[str, np.ndarray]]) -> Union[np.ndarray, Dict[str, np.ndarray]]:
        ...
        if self.norm_obs:
            if isinstance(obs, dict) and isinstance(self.obs_rms, dict):
                ...
                obs_ = []
                for i, ob in enumerate(obs):
                    obs_.append(self._unnormalize_obs(ob, self.obs_rms[i]).astype(np.float32))
                obs_ = tuple(obs_)
