    ...
    def step(self, action: Union[np.ndarray, int]) -> GymStepReturn:
        ...
        if done1:
            ...
            if not self.multiagent:
                ...
                ep_rew0 = sum([r[0] for r in self.rewards])
                ep_rew1 = sum([r[1] for r in self.rewards])
                ep_info = {"r0": round(ep_rew0, 6), "r1": round(ep_rew1, 6), "l": ep_len, "t": round(time.time() - self.t_start, 6)}
                for key in self.info_keywords:
                    ep_info[key] = info[key]
                self.episode_returns.append(ep_rew0 + ep_rew1)
            ...
            if self.multiagent:
                info[0]["episode"] = {k: ep_info[k] for k in ["r0", "l", "t"]}
                info[0]["episode"]["r"] = info[0]["episode"]["r0"]
                info[1]["episode"] = {k: ep_info[k] for k in ["r1", "l", "t"]}
                info[1]["episode"]["r"] = info[1]["episode"]["r1"]
            else:
...
class ResultsWriter:
    ...
    def __init__(
        ...
        if multiagent:
            self.logger = csv.DictWriter(self.file_handler, fieldnames=("r0", "r1", "l", "t") + extra_keys)
        else:
