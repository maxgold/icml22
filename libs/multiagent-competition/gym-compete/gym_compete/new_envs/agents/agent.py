...
class Agent(object):
    ...
    def in_agent_scope(self, name, agent_id):
        return name.startswith(f"agent{str(agent_id)}")
    ...
    def get_other_agent_qpos(self):
        ...
        for i in range(self.n_agents):
            ...
            qpos = self.env.data.qpos[startid: endid]
            other_qpos[i] = qpos
        return other_qpos
...
#        idx = self.body_ids[self.body_names.index(f"{self.scope}/{body_name}")]
#        import IPython as ipy; ipy.embed(colors='neutral')
#        return self.env.data.com_subtree[idx]
        return self.env.get_body_com(f"{self.scope}/{body_name}")

    def get_cfrc_ext(self):
        assert self._env_init, "Env reference is not set"
        try:
            raw_contact_forces = self.env.data.cfrc_ext[self.body_ids]
    ...
    def depricated_get_qpos(self):
        ...
        for j, start_idx in enumerate(self.jnt_qposadr):
            ...
            qpos[cnt: cnt + jlen] = self.env.data.qpos[start_idx: start_idx + jlen]
            cnt += jlen
        return qpos
    ...
    def get_torso_xmat(self):
        return self.env.data.body_xmat[self.body_ids[self.body_names.index(f'agent{self.id}/torso')]]

    # def get_ctrl(self):
    #     return self.env.data.ctrl[self.joint_ids]
