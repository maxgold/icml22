...
class KickAndDefend(MultiAgentEnv):
    ...
    def get_ball_qpos(self):
        ...
        return self.env_scene.data.qpos[start_idx:start_idx+self.jnt_nqpos]
    ...
    def get_ball_qvel(self):
        ...
        return self.env_scene.data.qvel[start_idx:start_idx+6]
    ...
    def get_ball_contacts(self, agent_id):
        ...
        for i in range(ncon):
            ...
            if g1.find('ball') >= 0:
                if g2.find(f"agent{str(agent_id)}") >= 0:
                    if ct.dist < 0:
    ...
    def _set_ball_xyz(self, xyz):
        ...
        qpos = self.env_scene.data.qpos.flatten().copy()
        qpos[start:start+3] = xyz
        qvel = self.env_scene.data.qvel.flatten()
    ...
    def _set_ball_vel(self, vel_xyz):
        ...
        qvel = self.env_scene.data.qvel.flatten().copy()
        qvel[start:start+len(vel_xyz)] = vel_xyz
        qpos = self.env_scene.data.qpos.flatten()
        self.env_scene.set_state(qpos, qvel)
