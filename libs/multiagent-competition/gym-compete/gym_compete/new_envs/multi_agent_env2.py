...
from .agents import *
from .utils import create_multiagent_xml_gym
import os
import six

class MultiAgentEnv2(Env):
    '''
    ...
    AGENT_MAP = {
        'ant': (
            os.path.join(os.path.dirname(__file__), "assets", "gym_ant.xml"),
    ...
    def __init__(
        ...
        all_agent_xml_paths = []
        if not agent_args:
            agent_args = [{"n_agents": self.n_agents} for _ in range(self.n_agents)]
...
#        else:
        ...
        _, self._env_xml_path = create_multiagent_xml_gym(
