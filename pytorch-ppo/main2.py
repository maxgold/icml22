...
def evaluate2(env, agent, video, num_episodes, step, args):
...
def main():
    ...
    envs = make_vec_envs(args.env_name, args.seed, args.num_processes,
                        ...
                        agent_names=['ant', 'ant'], init_pos=[(-1,0,.75), (1,0,.75)])
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            ...
            if args.multiagent:
                action = (action, torch.zeros(action.shape))
            obs, reward, done, infos = envs.step(action)
            if args.multiagent:
                obs, reward, done = obs[0], reward[0][0][0], done.any()
                done = [bool(done)]
                infos = [infos[0][0]]
                action = action[0]
